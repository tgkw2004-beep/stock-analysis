...
- `dart_fs_bs/cf/cis/is/sce`: 재무제표 (상태표, 현금흐름, 손익 등)
- `dart_fs_fscore`: Piotroski F-Score 분석 결과
- `dart_execshare_info` / `dart_maxshare_info`: 지분 현황
...
- `krx_stocks_ohlcv`: **일별 OHLCV (가장 많이 사용)**
- `krx_stocks_cap`: 시가총액 정보
- `krx_stocks_fundamental_info`: 투자지표 (PER, PBR 등)
- `krx_stocks_investor_shares_trading_info`: 투자자별 매매동향
