...
### 8.1 랭킹 리스트 카드 (Ranking List Card)
*   **용도**: 테마 수익률 TOP 5, ETF 가격 TOP 5 등 순위 기반 데이터를 표시할 때 사용합니다.
*   **아이콘/메달 규칙**: 
    - 1~3위: 금(`🥇`), 은(`🥈`), 동(`🥉`) 메달 이모지를 사용합니다.
    - 4~5위: 일반 숫자와 함께 브랜드 컬러 그라데이션(`.theme-rank-number`)을 배경으로 사용합니다.
*   **상태 표시**: 상승/하락 여부에 따라 카드 전체 테두리 또는 아이콘 색상을 `up`, `down` 클래스로 구분하여 시각적 피드백을 제공합니다.
*   **데이터 밀도**: 카드가 좁을 경우(Grid 레이아웃), 필수 정보(이름, 가격, 변동률)만 노출하고 세부 코드나 날짜는 작고 흐릿한 텍스트로 보조 처리합니다.
